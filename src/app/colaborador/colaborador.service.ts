...
export class ColaboradorService {
  ...
  updateStatus(id_colaborador: number, status: string): Observable<any> {
    return this.http
      .put(`${this.API}/${id_colaborador}`, { status_colaborador: status })
      .pipe(
        tap(() =>
          console.log(
            `Status do colaborador com ID ${id_colaborador} atualizado para ${status}`
          )
        )
      );
  }
  ...
  add(colaborador: Colaborador): Observable<Colaborador> {
    return this.http
      .post<Colaborador>(this.API, colaborador)
      .pipe(
        tap((novoColaborador) =>
          console.log('Novo colaborador adicionado:', novoColaborador)
        )
      );
  }
  ...
  update(
    ...
    return this.http
      .put<Colaborador>(`${this.API}/${id_colaborador}`, colaborador)
      .pipe(
        tap(() =>
          console.log(`Colaborador com ID ${id_colaborador} atualizado.`)
        )
      );
  }
  ...
  remove(id_colaborador: number): Observable<any> {
    return this.http
      .delete(`${this.API}/${id_colaborador}`)
      .pipe(
        tap(() => console.log(`Colaborador com ID ${id_colaborador} removido.`))
      );
  }

  // Enviar senha gerada por e-mail para o colaborador
  enviarSenha(id_colaborador: number): Observable<any> {
    return this.http
