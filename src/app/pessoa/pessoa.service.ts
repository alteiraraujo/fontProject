...
export class PessoaService {
  ...
  updateStatus(id_pessoa: number, status: string): Observable<any> {
    return this.http.put(`${this.API}/${id_pessoa}`, { status_pessoa: status }).pipe(
  ...
  update(id_pessoa: number, pessoa: Pessoa): Observable<Pessoa> {
    return this.http.put<Pessoa>(`${this.API}/${id_pessoa}`, pessoa).pipe(
      tap((response) => console.log(`Pessoa com ID ${id_pessoa} atualizada:`, response))
    );
  }

  buscarCEP(cep: string): Observable<any> {
    return this.http.get(`https://viacep.com.br/ws/${cep}/json/`).pipe(
      tap((response) => console.log('Dados do CEP buscados:', response))
    );
  }
}
